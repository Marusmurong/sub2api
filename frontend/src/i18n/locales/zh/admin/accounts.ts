export default {
    accounts: {
      ...
      reclaudeEditHint:
        '设备凭据（SK / seed / 设备 ID）建号后不可更换——禁止二次登录，换凭据等于重新采购一份订阅。这里只能改运维字段。',
      reclaudeBoundEmail: '当前绑定的 Claude 账号',
      reclaudeBoundEmailHint: '由上游静默切换，是会变的展示字段，不参与账号命名。',
      reclaudeCellCap: '日上限',
      reclaudeCellCapMissing: '未标定（不可调度）',
      reclaudeCellBound: '绑定',
      reclaudeCellSwitched: '换号',
      reclaudeCellClient: '版本',
      reclaudeSelfCheck: {
        title: 'reclaude 连通性自检',
        description: '三步全部通过后账号才会被置为可调度。失败不会改动账号当前状态。',
        run: '开始自检',
        running: '自检中…',
        failed: '自检请求失败',
        boundEmail: '当前绑定的 Claude 账号',
        activated: '三步全绿，账号已置为可调度。',
        notActivated: '未全部通过，账号状态保持不变。修复后可重新自检。',
        steps: {
          gateway_reachable: '① 网关节点可达',
          credential_valid: '② 设备凭据有效（SK 未被撤销）',
          envelope_signable: '③ 信封与签名链路通，且走的是绑定的那条代理'
        }
      },
      reclaudeSelfCheckAction: 'reclaude 自检',
      reclaudeUsageToday: '今日已用',
      reclaudeUsageUnavailable: '水位不可读',
      reclaudePasteLabel: '粘贴建号 JSON（推荐）',
      reclaudePastePlaceholder: '从本地控制台「生成建号 JSON」复制过来，粘贴即自动填表',
      reclaudePasteApply: '解析并填表',
      reclaudePasteHint: '八个字段会一次填好，避免手抄 SK 与 seed 两个长字符串。解析后原文会被清空。',
      reclaudePasteOk: '已填入，请核对后创建',
      reclaudePasteInvalid: '不是有效的 JSON',
      reclaudePasteRawJson: '这是原始 device.json —— 它的 gateway_url 是默认值而非 route 节点，请用控制台生成的建号 JSON',
      reclaudePasteMissing: '缺少字段：{fields}',
      reclaudeLabel: 'reclaude 中转订阅',
      reclaudeDesc: '设备凭据 sk + seed',
      reclaudeSectionTitle: 'reclaude 设备凭据',
      reclaudeSectionHint:
        '一行账号 = 一套设备凭据 = 一个配额包。凭据终身不可更换（禁止二次登录），录错只能重新采购一份订阅。',
      reclaudeSk: 'SK（Bearer 凭证）',
      reclaudeSkHint: 'sk-rec- 开头。加密存储，保存后不再回显。',
      reclaudeSeed: 'ed25519 Seed',
      reclaudeSeedHint: 'hex 或 base64，解码后必须恰好 32 字节。加密存储，保存后不再回显。',
      reclaudeDeviceId: '设备 ID',
      reclaudeFingerprint: '公钥指纹',
      reclaudeFingerprintHint: '8 字节 hex。推导算法尚未结案，暂不参与强校验。',
      reclaudeGatewayUrl: '网关节点',
      reclaudeGatewayUrlHint: '固定一个节点，不做自动选路——每次换边缘节点本身就是可识别的行为特征。',
      reclaudeClientVersion: '客户端版本',
      reclaudeClientVersionHint: '需要按月跟随官方版本，否则版本号会永远停在授权那天。',
      reclaudeClientPlatform: '客户端平台',
      reclaudeDeviceHostname: '设备 hostname',
      reclaudeDeviceHostnameHint: '用于生成账号名，并与 reclaude 设备页面对账。',
      reclaudeTimezone: '设备时区',
      reclaudeTimezoneHint: '决定「模拟关机」作息的生成时段。留空则用默认时区。',
      reclaudeUserEmail: '账号邮箱（选填）',
      reclaudeDailyTokenCap: '日 token 上限',
      reclaudeDailyTokenCapHint: '包络未标定（填 0）时禁止调度——未标定就售卖等于超卖。',
      reclaudeGeneratedName: '账号名（自动生成，不可编辑）',
      reclaudeProxyRequired: 'reclaude 账号必须绑定代理：没有代理就是直接用机房 IP 出网',
      reclaudeSkInvalid: 'SK 必须以 sk-rec- 开头',
      reclaudeSeedInvalid: 'seed 解码后必须恰好 32 字节',
      reclaudeDeviceIdInvalid: '设备 ID 必须是正整数',
      reclaudeDailyCapRequired: '日 token 上限必须大于 0',
      reclaudeGatewayInvalid: '网关地址必须是白名单内的 https 节点',
      reclaudeClientIdentityRequired: '客户端版本与平台都必须填写',
      ...
      sendingTestMessage: '发送测试消息："What does the git status command show?"',
