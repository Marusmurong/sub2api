export default {
    settings: {
      ...
      payment: {
        ...
        field_walletAddress: '收款钱包地址',
        field_network: '链 / 网络',
        field_tokenContract: 'USDT 合约地址',
        field_tronApiBase: 'TronGrid 环境',
        field_tronApiKey: 'TronGrid API Key',
        field_coingeckoApiBase: 'CoinGecko API 地址',
        field_coingeckoApiKey: 'CoinGecko API Key',
        field_ratePremiumPercent: '汇率溢价（%）',
        field_rateMaxStalenessSec: '汇率最大陈旧时间（秒）',
        field_usdtWalletAddressHint: 'TRC20 收款地址（T 开头 34 位）。保存时会校验 base58 校验和，但校验和正确不代表地址是你的 —— 请务必从钱包复制粘贴、逐字符核对，转错地址的资产无法找回。有待支付订单时不允许修改。',
        field_usdtTokenContractHint: 'USDT-TRC20 官方合约，通常无需修改。改成其他合约意味着只认那个代币的转账。',
        field_usdtTronApiKeyHint: 'TronGrid API Key，在 https://www.trongrid.io 注册后于控制台创建。用于查询到账流水，没有它无法自动核销。',
        field_usdtCoingeckoApiKeyHint: 'CoinGecko Demo API Key（https://www.coingecko.com/en/api/pricing 免费注册）。留空则走无 key 的公共限额，生产环境不建议。',
        field_usdtRatePremiumHint: '在 CoinGecko 市场价上叠加的溢价百分比。必须校准：CoinGecko 的 USDT/CNY 跟随美元官方牌价，通常比场外实际买入价低 2%–4%，不加溢价每笔都会亏。上线前对照实际 OTC 买入价调整，之后按月复核。',
        field_usdtRateStalenessHint: '汇率最大陈旧秒数（默认 1800）。超过此时长仍取不到新汇率时，USDT 渠道会自动停止接单，而不是拿一个不确定的旧汇率定价。',
        usdtDeposits: 'USDT 到账流水',
        usdtDepositsDesc: '每一笔打到收款地址的已确认转账。金额精确匹配的会自动核销；不匹配的（金额写错、迟到、误转）留在这里等人工处理。',
        field_cid: '支付渠道 ID',
