export default {
    settings: {
      ...
      payment: {
        ...
        field_walletAddress: 'Receiving wallet address',
        field_network: 'Chain / network',
        field_tokenContract: 'USDT contract address',
        field_tronApiBase: 'TronGrid environment',
        field_tronApiKey: 'TronGrid API key',
        field_coingeckoApiBase: 'CoinGecko API base',
        field_coingeckoApiKey: 'CoinGecko API key',
        field_ratePremiumPercent: 'Rate premium (%)',
        field_rateMaxStalenessSec: 'Max rate staleness (seconds)',
        field_usdtWalletAddressHint: 'TRC20 receiving address (34 chars, starts with T). The base58 checksum is verified on save, but a valid checksum does not mean the address is yours — paste it from your wallet and check every character; funds sent to a wrong address cannot be recovered. Cannot be changed while orders are pending.',
        field_usdtTokenContractHint: 'The official USDT-TRC20 contract; rarely needs changing. Pointing it elsewhere means only that token will be recognised.',
        field_usdtTronApiKeyHint: 'TronGrid API key, created in the dashboard after registering at https://www.trongrid.io. Used to read incoming transfers; without it nothing can be reconciled automatically.',
        field_usdtCoingeckoApiKeyHint: 'CoinGecko Demo API key (free at https://www.coingecko.com/en/api/pricing). Leaving it blank falls back to the keyless public rate limit, which is not suitable for production.',
        field_usdtRatePremiumHint: 'Markup applied over the CoinGecko market rate. This must be calibrated: CoinGecko tracks the official USD/CNY reference, which typically sits 2-4% below the real OTC cost of USDT, so a zero premium loses money on every order. Set it against live OTC quotes before going live and review monthly.',
        field_usdtRateStalenessHint: 'Maximum age of a cached rate in seconds (default 1800). Once the last good rate is older than this, the USDT method stops accepting orders rather than pricing them against a rate nobody can vouch for.',
        usdtDeposits: 'USDT deposits',
        usdtDepositsDesc: 'Every confirmed transfer that reached the receiving address. Exact-amount matches settle automatically; anything else (wrong amount, late, mistaken transfer) waits here for an operator.',
