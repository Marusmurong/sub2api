export default {
    accounts: {
      ...
      reclaudeEditHint:
        'Device credentials (SK / seed / device ID) cannot be rotated after creation — re-login is disallowed, so replacing them means buying a new subscription. Only operational fields are editable here.',
      reclaudeBoundEmail: 'Currently bound Claude account',
      reclaudeBoundEmailHint: 'Switched silently upstream. A display-only field that changes over time; never part of the account name.',
      reclaudeCellCap: 'Daily cap',
      reclaudeCellCapMissing: 'uncalibrated (not schedulable)',
      reclaudeCellBound: 'Bound',
      reclaudeCellSwitched: 'Switched',
      reclaudeCellClient: 'Client',
      reclaudeSelfCheck: {
        title: 'reclaude connectivity self-check',
        description:
          'The account is only marked schedulable after all three steps pass. A failure leaves the current account state untouched.',
        run: 'Run self-check',
        running: 'Running...',
        failed: 'Self-check request failed',
        boundEmail: 'Currently bound Claude account',
        activated: 'All three steps passed; the account is now schedulable.',
        notActivated: 'Not all steps passed; the account state is unchanged. Fix the issue and run it again.',
        steps: {
          gateway_reachable: '1. Gateway node reachable',
          credential_valid: '2. Device credential valid (SK not revoked)',
          envelope_signable: '3. Envelope and signature path works, over the bound proxy'
        }
      },
      reclaudeSelfCheckAction: 'reclaude self-check',
      reclaudeUsageToday: 'Used today',
      reclaudeUsageUnavailable: 'usage unreadable',
      reclaudePasteLabel: 'Paste provisioning JSON (recommended)',
      reclaudePastePlaceholder: 'Copy from the local console\'s "generate provisioning JSON" and paste here',
      reclaudePasteApply: 'Parse and fill',
      reclaudePasteHint: 'Fills all eight fields at once, avoiding hand-copying the SK and seed. The pasted text is cleared after parsing.',
      reclaudePasteOk: 'Filled in — review before creating',
      reclaudePasteInvalid: 'Not valid JSON',
      reclaudePasteRawJson: 'This is a raw device.json — its gateway_url is the default, not a route node. Use the provisioning JSON from the console.',
      reclaudePasteMissing: 'Missing fields: {fields}',
      reclaudeLabel: 'reclaude relay',
      reclaudeDesc: 'device sk + seed',
      reclaudeSectionTitle: 'reclaude device credentials',
      reclaudeSectionHint:
        'One account row = one device credential set = one quota package. Credentials can never be rotated (re-login is not allowed); a mistake here means buying a new subscription.',
      reclaudeSk: 'SK (bearer credential)',
      reclaudeSkHint: 'Starts with sk-rec-. Stored encrypted and never shown again after saving.',
      reclaudeSeed: 'ed25519 seed',
      reclaudeSeedHint: 'Hex or base64; must decode to exactly 32 bytes. Stored encrypted and never shown again.',
      reclaudeDeviceId: 'Device ID',
      reclaudeFingerprint: 'Public key fingerprint',
      reclaudeFingerprintHint: '8-byte hex. The derivation algorithm is unconfirmed, so this is not hard-validated yet.',
      reclaudeGatewayUrl: 'Gateway node',
      reclaudeGatewayUrlHint: 'Pinned to one node; no auto-pick — hopping edge nodes is itself a recognizable behavior.',
      reclaudeClientVersion: 'Client version',
      reclaudeClientVersionHint: 'Must be bumped monthly, otherwise it stays frozen at the authorization date.',
      reclaudeClientPlatform: 'Client platform',
      reclaudeDeviceHostname: 'Device hostname',
      reclaudeDeviceHostnameHint: 'Used to build the account name and to reconcile with the reclaude device page.',
      reclaudeTimezone: 'Device timezone',
      reclaudeTimezoneHint: 'Drives the generated "simulated shutdown" schedule. Defaults when left blank.',
      reclaudeUserEmail: 'Account email (optional)',
      reclaudeDailyTokenCap: 'Daily token cap',
      reclaudeDailyTokenCapHint: 'A cap of 0 means the envelope is uncalibrated and the account will not be scheduled.',
      reclaudeGeneratedName: 'Account name (generated, read-only)',
      reclaudeProxyRequired: 'A reclaude account must be bound to a proxy; without one it egresses from the datacenter IP',
      reclaudeSkInvalid: 'SK must start with sk-rec-',
      reclaudeSeedInvalid: 'The seed must decode to exactly 32 bytes',
      reclaudeDeviceIdInvalid: 'Device ID must be a positive integer',
      reclaudeDailyCapRequired: 'Daily token cap must be greater than 0',
      reclaudeGatewayInvalid: 'The gateway must be an allowlisted https node',
      reclaudeClientIdentityRequired: 'Client version and platform are both required',
      ...
      sendingTestMessage: 'Sending test message: "What does the git status command show?"',
