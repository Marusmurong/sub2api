...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('admin.usage.requestId'), t('admin.usage.upstreamRequestId'), t('admin.usage.upstreamTraceId'), t('usage.userAgent'), t('admin.usage.ipAddress')
    ]
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.request_id || '', log.upstream_request_id || '', log.upstream_trace_id || '', log.user_agent || '', log.ip_address || ''
      ])
      if (rows.length) {
...
const DEFAULT_HIDDEN_COLUMNS = ['reasoning_effort', 'request_id', 'upstream_request_id', 'upstream_trace_id', 'user_agent']
...
const HIDDEN_COLUMNS_CURRENT_VERSION = 'upstream-trace-id-hidden-by-default'
...
const allColumns = computed(() => [
  ...
  { key: 'upstream_trace_id', label: t('admin.usage.upstreamTraceId'), sortable: false },
  ...
  try {
    ...
    if (saved) {
      ...
      if (savedVersion !== HIDDEN_COLUMNS_CURRENT_VERSION) {
        ...
        hiddenColumns.add('upstream_trace_id')
        localStorage.setItem(HIDDEN_COLUMNS_KEY, JSON.stringify([...hiddenColumns]))
        localStorage.setItem(HIDDEN_COLUMNS_VERSION_KEY, HIDDEN_COLUMNS_CURRENT_VERSION)
      }
    } else {
