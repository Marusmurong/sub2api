      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button
              v-if="account.type === 'reclaude'"
              @click="$emit('reclaude-self-check', account); $emit('close')"
              class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700"
            >
              <Icon name="check" size="sm" class="text-teal-500" />
              {{ t('admin.accounts.reclaudeSelfCheckAction') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow', 'reclaude-self-check'])
const { t } = useI18n()
