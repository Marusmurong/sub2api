...
func TestBuildUpstreamRequestOAuthMimic_MidConversationOutputConfig(t *testing.T) {
	cases := []struct {
		...
		// 与上游有意分歧：mimic 的 beta 集合以真实客户端抓包为准（2026-09-07 本机
		// 2.1.263 OAuth 抓样里没有 mid-conversation-output-config-2026-07-01），
		// 所以 mimic 默认**不**注入该 beta，空控制 system 消息随之被整条删除。
		// 净效果与改动前一致：此前这条消息本就被 hoistSystemRoleMessages 提升掉了。
		// 重抓样本若发现该 beta，把首行改回 {..., true, 2, true} 并加进三族模板。
		{"default_mimic_drops_beta_per_capture", false, false, 1, false},
