...
func TestGetClaudeCodeClientVersionPriority(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "手动值优先于未标定的同步值", manual: "2.1.280", synced: "2.1.281", want: "2.1.280"},
		{name: "归一化手动值", manual: " v2.1.280 ", synced: "2.1.280", want: "2.1.280"},
		{name: "跟随已标定的同步值", synced: "2.1.280", want: "2.1.280"},
		{name: "无效手动值回退到同步值", manual: "invalid", synced: "2.1.280", want: "2.1.280"},
...
func TestUpdateSettingsClaudeCodeVersionTakesEffectImmediately(t *testing.T) {
	...
	require.Equal(t, "2.1.280", svc.GetClaudeCodeClientVersion(ctx))
	...
	require.Equal(t, "2.1.280", repo.values[SettingKeyClaudeCodeClientVersionSynced])
