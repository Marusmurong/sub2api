...
func TestBuildOAuthMimicRequest_RuntimeVersionConsistentBetweenHeaderAndBilling(t *testing.T) {
	...
	// 🔴 必须用**已标定**的版本：未标定版本会被 IsSupportedCLIVersion 拒绝
	// （见 pkg/claude/cli_version.go 的 CalibratedCLIVersions），resolver 返回值
	// 不生效，本用例断言的「运行期注入值」就不会出现。
	// 上游原本用 9.9.9，那在我们这里是个会被闸挡掉的未标定版本。
	upgraded := claude.CalibratedCLIVersions[len(claude.CalibratedCLIVersions)-1]
...
func TestIsAcceptableFingerprintUserAgent_UsesRuntimeVersion(t *testing.T) {
	...
	// 超前量按运行期版本的主版本号算（上游原本硬编码 11/12，那是按 9.9.9 推的）。
	major := strings.SplitN(upgraded, ".", 2)[0]
	majorNum, err := strconv.Atoi(major)
	require.NoError(t, err)
	// 允许的最大主版本超前量是 +2。
	require.True(t, isAcceptableFingerprintUserAgent(
		fmt.Sprintf("claude-cli/%d.0.0 (external, cli)", majorNum+2)))
	// 超前 +3 会被拒（与静态基线下的既有语义一致，只是基准换成运行期版本）。
	require.False(t, isAcceptableFingerprintUserAgent(
		fmt.Sprintf("claude-cli/%d.0.0 (external, cli)", majorNum+3)))
}
