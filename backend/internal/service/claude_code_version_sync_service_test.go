...
func TestClaudeCodeVersionSyncWritesLatestStableVersion(t *testing.T) {
	repo := newClaudeCodeVersionSyncSettingRepoStub(map[string]string{SettingKeyClaudeCodeVersionAutoSyncEnabled: "true"})
...
// 🔴 默认前提已反转（2026-09-23 合并 v0.2.8）：开关缺失/空值/读取失败一律视为
// **关闭**，与 setting_parse.go 的默认值一致。见 autoSyncEnabled 的说明 ——
// 同步只推进版本号不动 SDK/beta/TLS，拉到未标定版本会发出不存在的字段组合。
func TestClaudeCodeVersionSyncDisabledByDefaultOrOnError(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Empty(t, repo.syncedWrites(), "默认应关闭，不该写入同步值")
		})
	}
}

// 显式开启后同步逻辑照常工作。
func TestClaudeCodeVersionSyncWorksWhenExplicitlyEnabled(t *testing.T) {
	repo := newClaudeCodeVersionSyncSettingRepoStub(map[string]string{
		SettingKeyClaudeCodeVersionAutoSyncEnabled: "true",
	})
	github := &claudeCodeVersionSyncGitHubStub{releases: []*GitHubRelease{{TagName: "v2.1.280"}}}
...
func TestClaudeCodeVersionSyncInitialRunsWhenStaleOrMissing(t *testing.T) {
	t.Run("同步值已过期时会重新拉取", func(t *testing.T) {
		repo := newClaudeCodeVersionSyncSettingRepoStub(map[string]string{
			SettingKeyClaudeCodeVersionAutoSyncEnabled: "true",
		})
	...
	// 🔴 上游拉到未标定版本时不得写入 —— 这正是自动同步的主要风险：
	// 它只推进版本号，不动 SDK/beta/TLS，组合起来就是真实世界不存在的客户端。
	// 见 claude.CalibratedCLIVersions。
	t.Run("上游的新版本未标定时不写入", func(t *testing.T) {
		repo := newClaudeCodeVersionSyncSettingRepoStub(map[string]string{
			SettingKeyClaudeCodeVersionAutoSyncEnabled: "true",
		})
		repo.updatedAt = time.Now().Add(-2 * time.Hour)
		...
		require.Empty(t, repo.syncedWrites(), "未标定版本不该进入出站身份")
	})
