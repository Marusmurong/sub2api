...
// autoSyncEnabled 读取面板开关。
//
// 🔴 默认**关闭**（上游默认开启，这里刻意反过来）。
//
// 同步服务只推进版本号，不动 X-Stainless-Package-Version / Runtime-Version /
// beta 模板 / TLS profile。拉到一个我们没抓过包的版本时，发出的是
// 「新版本号 + 旧 SDK + 旧 beta」——真实世界不存在的组合。版本号落后只是
// 「旧客户端」，字段组合错位是「伪造客户端」，后者严重得多，且失败完全静默。
//
// claude.CalibratedCLIVersions 已是第一道闸（未标定版本不生效），这里是第二道：
// 即便将来标定集合放宽，也不该由一个后台定时任务替我们决定出站身份。
//
// 读取失败同样按关闭处理——上游选的是「失败保持开启，避免数据库抖动停掉版本跟随」，
// 对我们相反：宁可不跟随，也不要在状态不确定时改出站身份。
