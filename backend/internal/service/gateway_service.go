...
type GatewayService struct {
	...
	// 按客户端平台分池（client_platform_pool.go）：观察计数存储与目标占比缓存。
	clientPlatformStats        ClientPlatformStatsCache
	clientPlatformSharesMu     sync.Mutex
	clientPlatformSharesCached map[ClientPlatform]float64
	clientPlatformSharesExpiry time.Time
