...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (result *ForwardResult, err error) {
	...
	lastWireBody := body
	retryStart := time.Now()
	// 本次调用内对该账号的出站序号（含循环内的补救重发），驱动 X-Stainless-Retry-Count
	// 像真实 SDK 一样逐次递增（审计 H-4）。跨账号故障转移会重新进入 Forward，从 1 计。
	wireAttempt := 0
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		// 构建上游请求（每次重试需要重新构建，因为请求体需要重新读取）
		upstreamCtx, releaseUpstreamCtx := detachStreamUpstreamContext(nextUpstreamAttemptCtx(ctx, &wireAttempt), reqStream)
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
					...
					retryCtx, releaseRetryCtx := detachStreamUpstreamContext(nextUpstreamAttemptCtx(ctx, &wireAttempt), reqStream)
						...
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
									...
									UpstreamRequestID:  upstreamRequestID(retryResp.Header),
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									retryCtx2, releaseRetryCtx2 := detachStreamUpstreamContext(nextUpstreamAttemptCtx(ctx, &wireAttempt), reqStream)
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						budgetRetryCtx, releaseBudgetRetryCtx := detachStreamUpstreamContext(nextUpstreamAttemptCtx(ctx, &wireAttempt), reqStream)
				...
				if claude.IsSamplingParamsDeprecatedError(errMsg) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						samplingRetryCtx, releaseSamplingRetryCtx := detachStreamUpstreamContext(nextUpstreamAttemptCtx(ctx, &wireAttempt), reqStream)
	...
	// 记录本轮 upstream request id，供同一会话下一轮作 cc_prev_req parent-link。
	// 放在此处而非各响应处理器内部：这里是"上游已接受"的唯一收口，且失败重试
	// 走不到，天然满足"只在成功路径记录"。
	s.rememberUpstreamRequestID(ctx, c, account, resp.Header)
		...
		if err != nil {
			...
			if errors.As(err, &sseErr) {
				...
				logger.LegacyPrintf("service.gateway",
					...
					account.ID, account.Name, upstreamRequestID(resp.Header),
	...
	return &ForwardResult{
		// RequestID 用 upstreamRequestID 而不是 resp.Header.Get("x-request-id")：
