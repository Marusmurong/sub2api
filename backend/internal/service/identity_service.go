...
//
// 🔴 结构采纳上游 v0.2.8：UserAgent 不能在包 init 时固化 —— 必须每次使用时经
// EffectiveCLIVersion 现取，否则运行期版本变化后，新建账号会把过期版本写成持久身份
// （指纹「只升不降」且系统内没有重置入口）。
//
// 🔴 取值保留我们的 2026-09-07 抓包：上游那份仍是 Package-Version 0.94.0 /
// Runtime v24.3.0。这几项与 UA 版本号是一组耦合的取值，换任何一项都要重新抓包。
...
//
// 强制身份路径不经过 UA 校验：它根本不从客户端头派生，无从被毒化。校验只针对
// 下面这条会吸收客户端 UA 的遗留路径。
