...
// bumpPatch 返回比内置基线高一个补丁位的版本号。
//
// 上游原用写死的 "2.1.251" 表达「高于基线」，那是相对 2.1.220 才成立的；
// 我们的基线是 2.1.257（fable-5-1 要求 >= 2.1.251），写死值会被正确地判为
// 低于基线而使用例失败。改为从 CLICurrentVersion 推导，基线再变也不用改测试。
func bumpPatch(t *testing.T, version string) string {
	t.Helper()
	parts := strings.Split(version, ".")
	if len(parts) != 3 {
		t.Fatalf("CLICurrentVersion %q 不是三段 semver", version)
	}
	patch, err := strconv.Atoi(parts[2])
	if err != nil {
		t.Fatalf("CLICurrentVersion %q 补丁位不是数字: %v", version, err)
	}
	return fmt.Sprintf("%s.%s.%d", parts[0], parts[1], patch+1)
}

func TestIsSupportedCLIVersion(t *testing.T) {
	above := bumpPatch(t, CLICurrentVersion)
	cases := []struct {
		name    string
		version string
		want    bool
